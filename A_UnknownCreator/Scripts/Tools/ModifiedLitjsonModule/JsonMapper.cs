...
namespace UnknownCreator.Modules
{
    ...
    public class JsonMapper
    {
        ...
        private static readonly object custom_exporters_table_lock = new System.Object();

        private static readonly IDictionary<Type,
                IDictionary<Type, ImporterFunc>> base_importers_table;
        private static readonly IDictionary<Type,
                IDictionary<Type, ImporterFunc>> custom_importers_table;
        private static readonly object custom_importers_table_lock = new System.Object();
        ...
        private const string TypeTitle = "_T";
        #endregion
        ...
        private static bool IsSkipField(FieldInfo info)
        {
            if (Attribute.IsDefined(info, typeof(JsonIgnoreAttribute)))
                return true;

            if (!info.IsPublic && !Attribute.IsDefined(info, typeof(JsonMarkAttribute)))
                return true;

            return false;
        }

        private static bool IsSkipProperty(PropertyInfo info)
        {
            if (Attribute.IsDefined(info, typeof(JsonIgnoreAttribute)))
                return true;

            MethodInfo get, set;
            if (!Attribute.IsDefined(info, typeof(JsonMarkAttribute)))
            {
                if (!info.CanRead || !info.CanWrite)
                    return true;

                get = info.GetGetMethod(true);
                set = info.GetSetMethod(true);
                if (((get != null && !get.IsPublic) &&
                    (set != null && !set.IsPublic)))
                    return true;
            }
        ...
        private static void AddArrayMetadata(Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static))
            {
                if (IsSkipProperty(p_info))
                    continue;
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            foreach (FieldInfo f_info in type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static))
            {
                if (IsSkipField(f_info))
                    continue;
        ...
        private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
        {
            ...
            if (obj is IDictionary dictionary)
            {
                ...
                writer.WritePropertyName(TypeTitle);
                writer.Write(dictionary.GetType().GetFullTypeName());
        ...
        private static object ReadValue(Type inst_type, JsonReader reader)
        {
            ...
            {
#if NETSTANDARD1_5
                if (inst_type.IsClass() || inst_type.IsInterface() || underlying_type != null) {
                    return null;
                }
#else
                if (inst_type.IsClass || inst_type.IsInterface || underlying_type != null)
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                while (true)
                {
                    ...
                    instance ??= OnObjectCreate == null ? Activator.CreateInstance(value_type) : OnObjectCreate(value_type);


                    if (t_data.Properties.TryGetValue(property, out PropertyMetadata prop_data))
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var converter = TypeDescriptor.GetConverter(dicTypes[0]);
                            if (converter != null)
        ...
        private static void RegisterBaseImporters()
        {
            ...
            RegisterImporter(typeof(int), typeof(byte), delegate (object input)
            {
                return Convert.ToByte((int)input);
            });


            RegisterImporter(typeof(int), typeof(ulong), delegate (object input)
            {
                return Convert.ToUInt64((int)input);
            });


            RegisterImporter(typeof(int), typeof(long), delegate (object input)
            {
                return Convert.ToInt64((int)input);
            });


            RegisterImporter(typeof(int), typeof(sbyte), delegate (object input)
            {
                return Convert.ToSByte((int)input);
            });


            RegisterImporter(typeof(int), typeof(short), delegate (object input)
            {
                return Convert.ToInt16((int)input);
            });


            RegisterImporter(typeof(int), typeof(ushort), delegate (object input)
            {
                return Convert.ToUInt16((int)input);
            });


            RegisterImporter(typeof(int), typeof(uint), delegate (object input)
            {
                return Convert.ToUInt32((int)input);
            });


            RegisterImporter(typeof(int), typeof(float), delegate (object input)
            {
                return Convert.ToSingle((int)input);
            });


            RegisterImporter(typeof(int), typeof(double), delegate (object input)
            {
                return Convert.ToDouble((int)input);
            });


            RegisterImporter(typeof(double), typeof(decimal), delegate (object input)
            {
                return Convert.ToDecimal((double)input);
            });


            RegisterImporter(typeof(double), typeof(float), delegate (object input)
            {
                return Convert.ToSingle((double)input);
            });


            RegisterImporter(typeof(long), typeof(uint), delegate (object input)
            {
                return Convert.ToUInt32((long)input);
            });


            RegisterImporter(typeof(string), typeof(char), delegate (object input)
            {
                return Convert.ToChar((string)input);
            });


            RegisterImporter(typeof(string), typeof(DateTime), delegate (object input)
            {
                return Convert.ToDateTime((string)input, datetime_format);
            });


            RegisterImporter(typeof(string), typeof(DateTimeOffset), delegate (object input)
            {
        ...
        private static void RegisterImporter(Type json_type, Type value_type, ImporterFunc importer)
        {
            if (!base_importers_table.ContainsKey(json_type))
                base_importers_table.Add(json_type, new Dictionary<Type, ImporterFunc>());

            base_importers_table[json_type][value_type] = importer;
        }
